...
export const RollStateList: React.FC<Props> = ({ stateList, size = 14, onItemClick, studentList }) => {
  ...
  return (
    <S.ListContainer>
      {stateList.map((s, i) => {
        ...
        return (
          <S.ListItem key={i}>
            <RollStateIcon type={s.type} size={size} studentList={studentList} onClick={() => onClick(s.type)} />
            <span>{s.count}</span>
          </S.ListItem>
        )
      })}
